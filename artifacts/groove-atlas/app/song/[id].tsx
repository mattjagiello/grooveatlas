...
const ENERGY_LEVELS = ['LOW', 'MEDIUM', 'HIGH', 'VERY_HIGH'];

function EnergyBar({ level, colors }: { level: string | null; colors: ReturnType<typeof useColors> }) {
  const idx = level ? ENERGY_LEVELS.indexOf(level) : -1;
  return (
    <View style={styles.energyBarRow}>
      {ENERGY_LEVELS.map((_, i) => (
        <View
          key={i}
          style={[styles.energySegment, { backgroundColor: i <= idx ? '#EA580C' : colors.border }]}
        />
      ))}
    </View>
  );
...
type CyaniteState =
  | { phase: 'idle' }
  | { phase: 'loading' }
  | { phase: 'ready'; data: CyaniteAnalysis }
  | { phase: 'not_configured' }
  | { phase: 'no_spotify_id' }
  | { phase: 'error'; message: string };

function CyaniteResult({
  data,
  colors,
  onReset,
}: {
  data: CyaniteAnalysis;
  colors: ReturnType<typeof useColors>;
  onReset: () => void;
}) {
  const topMoods = data.moodTags.slice(0, 5);
  const allInstruments = [...data.instrumentTags, ...data.advancedInstrumentTags];
  const hasDrums = allInstruments.some((t) => /drum|percuss/i.test(t));

  return (
    <View style={styles.cyaniteResult}>
      {data.transformerCaption ? (
        <Text style={[styles.cyaniteCaption, { color: colors.foreground }]}>
          "{data.transformerCaption}"
        </Text>
      ) : null}

      <View style={styles.cyaniteRow}>
        <View style={styles.cyaniteStatBlock}>
          <Text style={[styles.cyaniteStatLabel, { color: colors.mutedForeground }]}>ENERGY</Text>
          <EnergyBar level={data.energyLevel} colors={colors} />
          <Text style={[styles.cyaniteStatSub, { color: colors.mutedForeground }]}>
            {[data.energyLevel, data.energyDynamics]
              .filter(Boolean)
              .map((s) => s!.replace(/_/g, ' '))
              .join(' · ')}
          </Text>
        ...
        {data.valence != null && data.arousal != null && (
          <View style={[styles.cyaniteEmotionGrid, { borderColor: colors.border }]}>
            <Text style={[styles.cyaniteGridLabel, { color: colors.mutedForeground }]}>FEEL</Text>
            <View style={styles.cyaniteGridArea}>
              <View
                style={[
                  styles.cyaniteGridDot,
                  {
                    backgroundColor: '#7C3AED',
                    left: Math.round(data.valence * 44),
                    top: Math.round((1 - data.arousal) * 44),
                  },
                ]}
              />
            </View>
            <View style={styles.cyaniteGridAxes}>
              <Text style={[styles.cyaniteGridAxisLabel, { color: colors.mutedForeground }]}>😔</Text>
              <Text style={[styles.cyaniteGridAxisLabel, { color: colors.mutedForeground }]}>😄</Text>
            </View>
          </View>
        )}
      ...
      {topMoods.length > 0 && (
        <View style={styles.cyaniteTagRow}>
          {topMoods.map((m) => {
            const color = MOOD_COLORS[m.toLowerCase()] ?? '#7C3AED';
            return (
              <View key={m} style={[styles.cyniteMoodTag, { backgroundColor: color + '22', borderColor: color + '55' }]}>
                <Text style={[styles.cyniteMoodText, { color }]}>{m.toLowerCase()}</Text>
              </View>
            );
          })}
        </View>
      ...
      <View style={styles.cyaniteMetaRow}>
        {data.bpm ? (
          <View style={styles.cyniteMiniStat}>
            <Text style={[styles.cyniteMiniValue, { color: colors.foreground }]}>{Math.round(data.bpm)}</Text>
            <Text style={[styles.cyniteMiniLabel, { color: colors.mutedForeground }]}>AI BPM</Text>
          </View>
        ) : null}
        {data.key ? (
          <View style={styles.cyniteMiniStat}>
            <Text style={[styles.cyniteMiniValue, { color: colors.foreground }]}>{data.key}</Text>
            <Text style={[styles.cyniteMiniLabel, { color: colors.mutedForeground }]}>Key</Text>
          </View>
        ) : null}
        {data.timeSignature ? (
          <View style={styles.cyniteMiniStat}>
            <Text style={[styles.cyniteMiniValue, { color: colors.foreground }]}>{data.timeSignature}</Text>
            <Text style={[styles.cyniteMiniLabel, { color: colors.mutedForeground }]}>Time Sig</Text>
          </View>
        ) : null}
        {data.musicalEraTag ? (
          <View style={styles.cyniteMiniStat}>
            <Text style={[styles.cyniteMiniValue, { color: colors.foreground }]}>{data.musicalEraTag}</Text>
            <Text style={[styles.cyniteMiniLabel, { color: colors.mutedForeground }]}>Era</Text>
          </View>
        ) : null}
        {hasDrums ? (
          <View style={[styles.cyniteMiniStat, { backgroundColor: '#7C3AED10', borderRadius: 6, paddingHorizontal: 6 }]}>
            <Text style={{ fontSize: 14 }}>🥁</Text>
            <Text style={[styles.cyniteMiniLabel, { color: '#7C3AED' }]}>Drums detected</Text>
          </View>
        ) : null}
      </View>

      <TouchableOpacity onPress={onReset} style={styles.resetBtn}>
        <Text style={[styles.resetText, { color: colors.mutedForeground }]}>↩ Re-analyze</Text>
      </TouchableOpacity>
    </View>
  );
}

function CyaniteCard({
  songId,
  spotifyId,
  colors,
}: {
  ...
  const [state, setState] = useState<CyaniteState>({ phase: 'idle' });

  const analyze = async () => {
    setState({ phase: 'loading' });
    const result = await fetchCyaniteAnalysis(songId, spotifyId);
    if (isCyaniteError(result)) {
      if (result.code === 'NOT_CONFIGURED') setState({ phase: 'not_configured' });
      else if (result.code === 'NO_SPOTIFY_ID') setState({ phase: 'no_spotify_id' });
      else setState({ phase: 'error', message: result.message });
      return;
    }
    setState({ phase: 'ready', data: result });
  };

  const reset = () => setState({ phase: 'idle' });

  return (
    <View style={[styles.cyaniteCard, { backgroundColor: colors.card, borderColor: '#7C3AED40' }]}>
      <View style={styles.cyaniteHeader}>
        <View style={[styles.cyaniteIconBadge, { backgroundColor: '#7C3AED20' }]}>
          <Text style={styles.cyaniteIconText}>✦</Text>
        </View>
        <View style={{ flex: 1 }}>
          <Text style={styles.cyaniteTitle}>AI Groove Analysis</Text>
          <Text style={[styles.cyaniteSubtitle, { color: colors.mutedForeground }]}>via Cyanite · mood, energy, instruments</Text>
        </View>
      </View>

      {state.phase === 'idle' && (
        <TouchableOpacity onPress={analyze} style={styles.cyaniteBtn}>
          <Text style={styles.cyniteBtnText}>✦  Analyze with AI</Text>
        </TouchableOpacity>
      )}

      {state.phase === 'loading' && (
        <View style={styles.cyaniteLoading}>
          <ActivityIndicator color="#7C3AED" />
          <Text style={[styles.cyaniteLoadingText, { color: colors.mutedForeground }]}>
            Analyzing audio fingerprint…
          </Text>
          <Text style={[styles.studyLoadingHint, { color: colors.mutedForeground }]}>
            First-time analysis takes ~15s per track
          </Text>
        </View>
      )}

      {state.phase === 'ready' && (
        <CyaniteResult data={state.data} colors={colors} onReset={reset} />
      )}

      {state.phase === 'not_configured' && (
        <View style={[styles.cyaniteNotice, { backgroundColor: '#7C3AED10', borderColor: '#7C3AED30' }]}>
          <Text style={[styles.cyaniteNoticeText, { color: colors.foreground }]}>
            Cyanite API access is being activated — check back after build week!
          </Text>
          <TouchableOpacity onPress={reset}>
            <Text style={[styles.resetText, { color: colors.mutedForeground }]}>Dismiss</Text>
          </TouchableOpacity>
      ...
      {state.phase === 'no_spotify_id' && (
        <View style={[styles.cyaniteNotice, { backgroundColor: colors.muted, borderColor: colors.border }]}>
          <Text style={[styles.cyaniteNoticeText, { color: colors.foreground }]}>
            Load track info first — tap "Track Info" above to fetch metadata.
          </Text>
          <TouchableOpacity onPress={reset}>
            <Text style={[styles.resetText, { color: colors.mutedForeground }]}>Dismiss</Text>
          </TouchableOpacity>
      ...
      {state.phase === 'error' && (
        <View style={[styles.cyaniteNotice, { backgroundColor: colors.muted, borderColor: colors.border }]}>
          <Text style={[styles.cyaniteNoticeText, { color: colors.foreground }]} numberOfLines={3}>
            {state.message}
          </Text>
          <TouchableOpacity onPress={reset}>
            <Text style={[styles.resetText, { color: colors.mutedForeground }]}>Retry</Text>
          </TouchableOpacity>
        </View>
      )}

      <Text style={[styles.cyaniteAttr, { color: colors.mutedForeground }]}>
        Powered by Cyanite AI · audio intelligence
      </Text>
    </View>
  );
}
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        {id && (
          <CyaniteCard
            songId={id}
            spotifyId={song.trackMeta?.spotifyId}
            colors={colors}
          />
        )}
...
const styles = StyleSheet.create({
  ...
  cyaniteCard: { marginHorizontal: 20, marginTop: 20, borderRadius: 12, borderWidth: 1, overflow: 'hidden' },
  cyaniteHeader: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 16, paddingBottom: 12 },
  cyaniteIconBadge: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
  cyaniteIconText: { fontSize: 16, color: '#7C3AED' },
  cyaniteTitle: { fontSize: 14, fontWeight: '700', color: '#7C3AED' },
  cyaniteSubtitle: { fontSize: 11, marginTop: 1 },
  cyaniteBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#7C3AED', marginHorizontal: 16, marginBottom: 16, paddingVertical: 12, borderRadius: 8 },
  cyniteBtnText: { color: '#fff', fontSize: 14, fontWeight: '700' },
  cyaniteLoading: { alignItems: 'center', gap: 8, paddingHorizontal: 16, paddingBottom: 16 },
  cyaniteLoadingText: { fontSize: 13, fontWeight: '500' },
  cyaniteNotice: { margin: 16, padding: 12, borderRadius: 8, borderWidth: 1, gap: 8, alignItems: 'center' },
  cyaniteNoticeText: { fontSize: 13, textAlign: 'center', lineHeight: 18 },
  cyaniteResult: { paddingHorizontal: 16, paddingBottom: 4, gap: 12 },
  cyaniteCaption: { fontSize: 13, lineHeight: 20, fontStyle: 'italic' },
  cyaniteRow: { flexDirection: 'row', gap: 12, alignItems: 'flex-start' },
  cyaniteStatBlock: { flex: 1, gap: 6 },
  cyaniteStatLabel: { fontSize: 9, fontWeight: '700', letterSpacing: 1.5 },
  energyBarRow: { flexDirection: 'row', gap: 3 },
  energySegment: { flex: 1, height: 8, borderRadius: 4 },
  cyaniteStatSub: { fontSize: 10 },
  cyaniteEmotionGrid: { width: 76, borderWidth: 1, borderRadius: 6, padding: 4, alignItems: 'center', gap: 2 },
  cyaniteGridLabel: { fontSize: 9, fontWeight: '700', letterSpacing: 1.5 },
  cyaniteGridArea: { width: 60, height: 60, borderRadius: 4, overflow: 'hidden', position: 'relative' },
  cyaniteGridDot: { position: 'absolute', width: 14, height: 14, borderRadius: 7 },
  cyaniteGridAxes: { flexDirection: 'row', width: 60, justifyContent: 'space-between' },
  cyaniteGridAxisLabel: { fontSize: 9 },
  cyaniteTagRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
  cyniteMoodTag: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, borderWidth: 1 },
  cyniteMoodText: { fontSize: 11, fontWeight: '600' },
  cyaniteMetaRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, alignItems: 'center' },
  cyniteMiniStat: { alignItems: 'center', gap: 2 },
  cyniteMiniValue: { fontSize: 15, fontWeight: '700' },
  cyniteMiniLabel: { fontSize: 9, fontWeight: '600', letterSpacing: 0.8 },
  cyaniteAttr: { fontSize: 10, textAlign: 'center', paddingBottom: 12, paddingTop: 4, letterSpacing: 0.3 },
});
